...
const AddMeeting = (props) => {
    ...
    const fetchAllData = async () => {
        try {
            ...
            if (!props.leadContect || props.leadContect === 'contactView') {
            ...
            if (!props.leadContect || props.leadContect === 'leadView') {
            ...
            toast.error("Failed to load contacts or leads");
        } finally {
            setIsLoding(false);
        }
    };
    ...
    useEffect(() => {
        ...
        if (props.id && props.leadContect) {
          if (props.leadContect === 'contactView' && contactList?.length > 0) {
            setContactData(contactList);
          } else if (props.leadContect === 'leadView' && leadData?.length > 0) {
